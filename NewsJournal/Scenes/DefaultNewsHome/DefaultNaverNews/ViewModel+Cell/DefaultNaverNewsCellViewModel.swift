...
final class DefaultNaverNewsCellViewModel {
    ...
    func retrieveImageFromLink(metaData: LPLinkMetadata, completionHandler: @escaping (UIImage?) -> Void ) {
        if let imageProvider = metaData.imageProvider {
           ...
           print("can't get imageProvider from metadata")
            completionHandler(nil)
        }
    ...
    func fetchNaverNewsMetaData(url: String, completionHandler: @escaping (Result<LPLinkMetadata, LPError>) -> Void ) {
